...
class AuthenticateService{
    async generateToken(user){
        const token =  jwt.sign({name:user.name, id:user._id, email:user.email}, process.env.SALT_KEY, { expiresIn: '1d' })
        return token
    }
    ...
    async isAdmin(req,res,next){
        ...
        if(decoded.roles.included('admin')){
            ...
            return res.status(403).json({message:'Only admins can use this route.'})
        }
        }
        })
    }
